...
def main():
	...
	parser.add_argument("-l", "--lib", required=True, help="lib name")
	...
	unset_all()
	if args.clean:
		cmd(["make","clean","LIB=%s" % args.lib, "BUILD_TARGET=%s" % build_target], args.verbose)
	cmd(["make","lib","LIB=%s" % args.lib, "BUILD_TARGET=%s" % build_target], args.verbose)
